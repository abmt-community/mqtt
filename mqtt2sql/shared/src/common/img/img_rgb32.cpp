...
template<> img_px<pixel_rgb32>::img_px(int32_t w, int32_t h, void* data_ptr, bool take_ptr_ownership){
	type = image_type::RGB32;
...
template<> img_px<pixel_rgb32>::img_px(img i){
	type = image_type::RGB32;
	width = i.width;
	height = i.height;
	...
	if(i.type == image_type::RGB8){
		...
		for(int x = 0; x < width; ++x ){
			for(int y = 0; y < height; ++y ){
				...
				at_2(x,y) = {  src_px.r/(float)255.0,  src_px.g/(float)255.0,  src_px.b/(float)255.0};
			}
		}
		...
		for(int x = 0; x < width; ++x ){
			for(int y = 0; y < height; ++y ){
				auto& src_px = src.at_2(x,y);
				at_2(x,y) = {  src_px/(float)255.0,  src_px/(float)255.0,  src_px/(float)255.0};
			}
		}
		...
		for(int pos = 0; pos < num_px; pos = pos+2){
			...
			dst_ptr[idx_arr+3] = (y2 + fr) <= 255 ? (y2 + fr)/255 : 1 ;
			dst_ptr[idx_arr+4] = (y2 + fg) <= 255 ? (y2 + fg)/255 : 1 ;
			dst_ptr[idx_arr+5] = (y2 + fb) <= 255 ? (y2 + fb)/255 : 1 ;
		}
