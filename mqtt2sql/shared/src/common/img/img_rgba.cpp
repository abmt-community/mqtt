...
template<> img_px<pixel_rgba>::img_px(int32_t w, int32_t h, void* data_ptr, bool take_ptr_ownership){
	type = image_type::RGBA8;
...
template<> img_px<pixel_rgba>::img_px(img i){
	type = image_type::RGBA8;
	width = i.width;
	height = i.height;
	...
	if(i.type == image_type::RGB8){
		...
		for(int x = 0; x < width; ++x ){
			for(int y = 0; y < height; ++y ){
				...
				at_2(x, y) = {src_px.r, src_px.g, src_px.b, 255};
			}
		}
		...
		for(int x = 0; x < width; ++x ){
			for(int y = 0; y < height; ++y ){
				auto& src_px = src.at_2(x,y);
				at_2(x, y) = {src_px, src_px, src_px, 255};
			}
		}
		...
		for(int x = 0; x < width; ++x ){
			for(int y = 0; y < height; ++y ){
				...
				uint8_t g = src_px.g <= 1 ? src_px.g*255 : 255;
				uint8_t b = src_px.b <= 1 ? src_px.b*255 : 255;
				uint8_t a = src_px.a <= 1 ? src_px.a*255 : 255;
				at_2(x, y) = {r, g, b, a};
			}
		}
		...
		for(int pos = 0; pos < num_px; pos = pos+2){
			...
			dst_ptr[idx_arr+3] = 255;

			dst_ptr[idx_arr+4] = (y2 + fr) <= 255 ? (y2 + fr) : 255 ;
			dst_ptr[idx_arr+5] = (y2 + fg) <= 255 ? (y2 + fg) : 255 ;
			dst_ptr[idx_arr+6] = (y2 + fb) <= 255 ? (y2 + fb) : 255 ;
			dst_ptr[idx_arr+7] = 255;
		}
	}else if(i.type == image_type::JPEG){
		int actual_comps;
		void* img_data = jpgd::decompress_jpeg_image_from_memory((const unsigned char*)i.data.data, i.data.size, &width, &height, &actual_comps, 4);
		int size = 4*width*height;
		if(img_data && actual_comps == 3){ // feature in jpgd: returns 3 when 4 requested, but it works;
